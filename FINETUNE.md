...
为了大幅提升训练速度并降低显存消耗，我们在微调前需要将音频文件（`.wav`）统一预处理编码为 PyTorch 的特征张量（`.pt`）。由于代码强制要求预处理，这一步是必须的。
...
python preprocess_latents.py \
    ...
    --output_jsonl "train_preprocessed.jsonl" \
    --output_dir "./data/precomputed_latents/"
```
执行完毕后，您会得到一个新的 `train_preprocessed.jsonl` 文件，里面的 `audio_path` 已经被替换成了 `latent_path`。

---

## 4. 开始全参微调

在项目根目录下，直接运行 `finetune.py` 启动脚本即可开始全参微调。请注意使用预处理后生成的 `train_preprocessed.jsonl`。

```bash
python finetune.py `
    --model_name_or_path "inclusionAI/Ming-omni-tts-0.5B" `
    --train_data_path "train_preprocessed.jsonl" `
